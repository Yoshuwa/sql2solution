...
public sealed partial class ProductDescriptionController : ControllerBase
{
    private readonly AppDbContext _db;

    public ProductDescriptionController(AppDbContext db)
    {
        _db = db;
    }

    partial void OnBeforeCreate(CreateProductDescriptionRequest request, ProductDescription item);
    partial void OnAfterCreate(ProductDescription item);
    partial void OnBeforeUpdate(ProductDescription item, UpdateProductDescriptionRequest request);
    partial void OnBeforeDelete(ProductDescription item);

    [HttpGet]
    public async Task<ActionResult<ApiResponse<PagedResult<ProductDescriptionDto>>>> GetAll([FromQuery] int page = 1, [FromQuery] int pageSize = 25, [FromQuery] string? search = null, [FromQuery] string? filterField = null, [FromQuery] string? filterValue = null, [FromQuery] string? sortBy = null, [FromQuery] string? sortDirection = null, CancellationToken ct = default)
    {
        page = Math.Max(1, page);
        pageSize = Math.Clamp(pageSize, 1, 200);
        IQueryable<ProductDescription> query = _db.Set<ProductDescription>().AsNoTracking();
        ...
        return Ok(ApiResponse<PagedResult<ProductDescriptionDto>>.Success("records loaded", new PagedResult<ProductDescriptionDto>(items, page, pageSize, total)));
    }

    [HttpGet("{id}")]
    public async Task<ActionResult<ApiResponse<ProductDescriptionDto>>> GetById(int id, CancellationToken ct)
    {
        IQueryable<ProductDescription> query = _db.Set<ProductDescription>().AsNoTracking();
        var item = await query.FirstOrDefaultAsync(x => x.ProductDescriptionID!.Equals(id), ct);
        return item is null ? NotFound(ApiResponse<object>.Warning("record not found")) : Ok(ApiResponse<ProductDescriptionDto>.Success("record loaded", ToDto(item)));
    }

    [HttpGet("{id}/history")]
    public async Task<ActionResult<ApiResponse<IReadOnlyList<AuditTrailDto>>>> GetHistory(int id, CancellationToken ct)
    {
        var canReadRecord = await _db.Set<ProductDescription>().AsNoTracking().AnyAsync(x => x.ProductDescriptionID!.Equals(id), ct);
        ...
        var history = await _db.AuditTrailEntries
            .AsNoTracking()
            .Where(entry => entry.Resource == "ProductDescription" && entry.ResourceKey == resourceKey)
    ...
    [HttpPost]
    public async Task<ActionResult<ApiResponse<ProductDescriptionDto>>> Create(CreateProductDescriptionRequest request, CancellationToken ct)
    {
        ...
        _db.Set<ProductDescription>().Add(item);
        try
        {
        ...
        await LogAuditTrailAsync("Created", Convert.ToString(item.ProductDescriptionID) ?? string.Empty, $"Created ProductDescription record {item.ProductDescriptionID}.", ToDto(item), ct);
        OnAfterCreate(item);
        return CreatedAtAction(nameof(GetById), new { id = item.ProductDescriptionID }, ApiResponse<ProductDescriptionDto>.Success("record created", ToDto(item)));
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, UpdateProductDescriptionRequest request, CancellationToken ct)
    {
        var item = await _db.Set<ProductDescription>().FirstOrDefaultAsync(x => x.ProductDescriptionID!.Equals(id), ct);
        ...
        await LogAuditTrailAsync("Updated", Convert.ToString(item.ProductDescriptionID) ?? string.Empty, $"Updated ProductDescription record {item.ProductDescriptionID}.", auditChanges, ct);
        return Ok(ApiResponse<object>.Success("record updated", new { updated = 1 }));
    }

    [HttpPatch("{id}")]
    public async Task<IActionResult> Patch(int id, UpdateProductDescriptionRequest request, CancellationToken ct)
    {
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        if (item is null) return NotFound(ApiResponse<object>.Warning("record not found"));
        OnBeforeDelete(item);
        _db.Set<ProductDescription>().Remove(item);
        try
        {
            await _db.SaveChangesAsync(ct);
        }
        ...
        await LogAuditTrailAsync("Deleted", Convert.ToString(id) ?? string.Empty, $"Hard deleted ProductDescription record {id}.", ToDto(item), ct);
        return Ok(ApiResponse<object>.Success("record deleted", new { deleted = 1, mode = "Hard" }));
    }

    [HttpPost("bulk/export")]
    public async Task<ActionResult<ApiResponse<PagedResult<ProductDescriptionDto>>>> ExportBulk(BulkIdsRequest request, [FromQuery] int page = 1, [FromQuery] int pageSize = 25, CancellationToken ct = default)
    {
        page = Math.Max(1, page);
        pageSize = Math.Clamp(pageSize, 1, 200);
        var ids = ParseBulkIds(request.Ids);
        if (ids.Count == 0) return Ok(ApiResponse<PagedResult<ProductDescriptionDto>>.Warning("no records selected", new PagedResult<ProductDescriptionDto>(Array.Empty<ProductDescriptionDto>(), page, pageSize, 0)));
        IQueryable<ProductDescription> query = _db.Set<ProductDescription>().AsNoTracking().Where(x => ids.Contains(x.ProductDescriptionID));
        var total = await query.CountAsync(ct);
        var items = await query.Skip((page - 1) * pageSize).Take(pageSize).Select(x => ToDto(x)).ToListAsync(ct);
        return Ok(ApiResponse<PagedResult<ProductDescriptionDto>>.Success("records exported", new PagedResult<ProductDescriptionDto>(items, page, pageSize, total)));
    }
    ...
    public async Task<IActionResult> UpdateBulk(BulkUpdateRequest request, CancellationToken ct)
    {
        ...
        IQueryable<ProductDescription> query = _db.Set<ProductDescription>().Where(x => ids.Contains(x.ProductDescriptionID));
        ...
        var auditChanges = items.ToDictionary(item => Convert.ToString(item.ProductDescriptionID) ?? string.Empty, item => GetEntityChanges(_db.Entry(item)));
        try
        {
            await _db.SaveChangesAsync(ct);
        }
        ...
        foreach (var item in items)
            await LogAuditTrailAsync("Updated", Convert.ToString(item.ProductDescriptionID) ?? string.Empty, $"Updated ProductDescription record {item.ProductDescriptionID} in bulk update.", auditChanges[Convert.ToString(item.ProductDescriptionID) ?? string.Empty], ct);
    ...
    public async Task<IActionResult> DeleteBulk(BulkIdsRequest request, CancellationToken ct)
    {
        ...
        _db.Set<ProductDescription>().RemoveRange(items);
        try
        {
            await _db.SaveChangesAsync(ct);
        }
        ...
        foreach (var item in items)
            await LogAuditTrailAsync("Deleted", Convert.ToString(item.ProductDescriptionID) ?? string.Empty, $"Hard deleted ProductDescription record {item.ProductDescriptionID} in bulk delete.", ToDto(item), ct);
    ...
    private static bool ApplyBulkUpdate(IReadOnlyList<ProductDescription> items, BulkUpdateRequest request, out string error)
    {
        error = string.Empty;
        return request.Field.Trim().ToLowerInvariant() switch
        {
            "description" => ApplyBulkDescription(items, request.Value, out error),
    ...
    private static bool ApplyBulkDescription(IReadOnlyList<ProductDescription> items, string? raw, out string error)
    {
        error = string.Empty;
        foreach (var item in items) item.Description = raw ?? string.Empty;
        return true;
    }

    private static bool ApplyBulkRowguid(IReadOnlyList<ProductDescription> items, string? raw, out string error)
    {
        error = string.Empty;
    ...
    private static bool ApplyBulkModifiedDate(IReadOnlyList<ProductDescription> items, string? raw, out string error)
    {
        error = string.Empty;
    ...
    private static IQueryable<ProductDescription> ApplySearch(IQueryable<ProductDescription> query, string? search)
    {
        if (string.IsNullOrWhiteSpace(search)) return query;
        search = search.Trim();
        return query.Where(x => (x.Description != null && x.Description.Contains(search)));
    }

    private static IQueryable<ProductDescription> ApplyFilter(IQueryable<ProductDescription> query, string? filterField, string? filterValue)
    {
        ...
        return filterField.ToLowerInvariant() switch
        {
            ...
            "description" => query.Where(x => x.Description != null && x.Description.Contains(filterValue)),
    ...
    private static IQueryable<ProductDescription> ApplySort(IQueryable<ProductDescription> query, string? sortBy, string? sortDirection)
    {
        ...
        var field = string.IsNullOrWhiteSpace(sortBy) ? "ProductDescriptionID" : sortBy.Trim();
        return field.ToLowerInvariant() switch
        {
            ...
            _ => descending ? query.OrderByDescending(x => x.ProductDescriptionID) : query.OrderBy(x => x.ProductDescriptionID)
        };
    }
    private static ProductDescriptionDto ToDto(ProductDescription item) => new(
