...
public sealed partial class ProductModelController : ControllerBase
{
    private readonly AppDbContext _db;

    public ProductModelController(AppDbContext db)
    {
        _db = db;
    }

    partial void OnBeforeCreate(CreateProductModelRequest request, ProductModel item);
    partial void OnAfterCreate(ProductModel item);
    partial void OnBeforeUpdate(ProductModel item, UpdateProductModelRequest request);
    partial void OnBeforeDelete(ProductModel item);

    [HttpGet]
    public async Task<ActionResult<ApiResponse<PagedResult<ProductModelDto>>>> GetAll([FromQuery] int page = 1, [FromQuery] int pageSize = 25, [FromQuery] string? search = null, [FromQuery] string? filterField = null, [FromQuery] string? filterValue = null, [FromQuery] string? sortBy = null, [FromQuery] string? sortDirection = null, CancellationToken ct = default)
    {
        page = Math.Max(1, page);
        pageSize = Math.Clamp(pageSize, 1, 200);
        IQueryable<ProductModel> query = _db.Set<ProductModel>().AsNoTracking();
        ...
        return Ok(ApiResponse<PagedResult<ProductModelDto>>.Success("records loaded", new PagedResult<ProductModelDto>(items, page, pageSize, total)));
    }

    [HttpGet("{id}")]
    public async Task<ActionResult<ApiResponse<ProductModelDto>>> GetById(int id, CancellationToken ct)
    {
        IQueryable<ProductModel> query = _db.Set<ProductModel>().AsNoTracking();
        var item = await query.FirstOrDefaultAsync(x => x.ProductModelID!.Equals(id), ct);
        return item is null ? NotFound(ApiResponse<object>.Warning("record not found")) : Ok(ApiResponse<ProductModelDto>.Success("record loaded", ToDto(item)));
    }

    [HttpGet("{id}/history")]
    public async Task<ActionResult<ApiResponse<IReadOnlyList<AuditTrailDto>>>> GetHistory(int id, CancellationToken ct)
    {
        var canReadRecord = await _db.Set<ProductModel>().AsNoTracking().AnyAsync(x => x.ProductModelID!.Equals(id), ct);
        ...
        var history = await _db.AuditTrailEntries
            .AsNoTracking()
            .Where(entry => entry.Resource == "ProductModel" && entry.ResourceKey == resourceKey)
    ...
    [HttpPost]
    public async Task<ActionResult<ApiResponse<ProductModelDto>>> Create(CreateProductModelRequest request, CancellationToken ct)
    {
        ...
        await LogAuditTrailAsync("Created", Convert.ToString(item.ProductModelID) ?? string.Empty, $"Created ProductModel record {item.ProductModelID}.", ToDto(item), ct);
        OnAfterCreate(item);
        return CreatedAtAction(nameof(GetById), new { id = item.ProductModelID }, ApiResponse<ProductModelDto>.Success("record created", ToDto(item)));
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, UpdateProductModelRequest request, CancellationToken ct)
    {
        var item = await _db.Set<ProductModel>().FirstOrDefaultAsync(x => x.ProductModelID!.Equals(id), ct);
        if (item is null) return NotFound(ApiResponse<object>.Warning("record not found"));
        OnBeforeUpdate(item, request);
        item.Name = request.Name;
        item.CatalogDescription = request.CatalogDescription;
        ...
        await LogAuditTrailAsync("Updated", Convert.ToString(item.ProductModelID) ?? string.Empty, $"Updated ProductModel record {item.ProductModelID}.", auditChanges, ct);
        return Ok(ApiResponse<object>.Success("record updated", new { updated = 1 }));
    }

    [HttpPatch("{id}")]
    public async Task<IActionResult> Patch(int id, UpdateProductModelRequest request, CancellationToken ct)
    {
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        if (item is null) return NotFound(ApiResponse<object>.Warning("record not found"));
        OnBeforeDelete(item);
        _db.Set<ProductModel>().Remove(item);
        try
        {
            await _db.SaveChangesAsync(ct);
        }
        ...
        await LogAuditTrailAsync("Deleted", Convert.ToString(id) ?? string.Empty, $"Hard deleted ProductModel record {id}.", ToDto(item), ct);
        return Ok(ApiResponse<object>.Success("record deleted", new { deleted = 1, mode = "Hard" }));
    }

    [HttpPost("bulk/export")]
    public async Task<ActionResult<ApiResponse<PagedResult<ProductModelDto>>>> ExportBulk(BulkIdsRequest request, [FromQuery] int page = 1, [FromQuery] int pageSize = 25, CancellationToken ct = default)
    {
        page = Math.Max(1, page);
        pageSize = Math.Clamp(pageSize, 1, 200);
        var ids = ParseBulkIds(request.Ids);
        if (ids.Count == 0) return Ok(ApiResponse<PagedResult<ProductModelDto>>.Warning("no records selected", new PagedResult<ProductModelDto>(Array.Empty<ProductModelDto>(), page, pageSize, 0)));
        IQueryable<ProductModel> query = _db.Set<ProductModel>().AsNoTracking().Where(x => ids.Contains(x.ProductModelID));
        var total = await query.CountAsync(ct);
        var items = await query.Skip((page - 1) * pageSize).Take(pageSize).Select(x => ToDto(x)).ToListAsync(ct);
        return Ok(ApiResponse<PagedResult<ProductModelDto>>.Success("records exported", new PagedResult<ProductModelDto>(items, page, pageSize, total)));
    }
    ...
    public async Task<IActionResult> UpdateBulk(BulkUpdateRequest request, CancellationToken ct)
    {
        ...
        IQueryable<ProductModel> query = _db.Set<ProductModel>().Where(x => ids.Contains(x.ProductModelID));
        ...
        var auditChanges = items.ToDictionary(item => Convert.ToString(item.ProductModelID) ?? string.Empty, item => GetEntityChanges(_db.Entry(item)));
        try
        {
            await _db.SaveChangesAsync(ct);
        }
        ...
        foreach (var item in items)
            await LogAuditTrailAsync("Updated", Convert.ToString(item.ProductModelID) ?? string.Empty, $"Updated ProductModel record {item.ProductModelID} in bulk update.", auditChanges[Convert.ToString(item.ProductModelID) ?? string.Empty], ct);
    ...
    public async Task<IActionResult> DeleteBulk(BulkIdsRequest request, CancellationToken ct)
    {
        ...
        _db.Set<ProductModel>().RemoveRange(items);
        try
        {
            await _db.SaveChangesAsync(ct);
        }
        ...
        foreach (var item in items)
            await LogAuditTrailAsync("Deleted", Convert.ToString(item.ProductModelID) ?? string.Empty, $"Hard deleted ProductModel record {item.ProductModelID} in bulk delete.", ToDto(item), ct);
    ...
    private static bool ApplyBulkUpdate(IReadOnlyList<ProductModel> items, BulkUpdateRequest request, out string error)
    {
        error = string.Empty;
        return request.Field.Trim().ToLowerInvariant() switch
        {
            "name" => ApplyBulkName(items, request.Value, out error),
            "catalogdescription" => ApplyBulkCatalogDescription(items, request.Value, out error),
    ...
    private static bool ApplyBulkName(IReadOnlyList<ProductModel> items, string? raw, out string error)
    {
        error = string.Empty;
    ...
    private static bool ApplyBulkCatalogDescription(IReadOnlyList<ProductModel> items, string? raw, out string error)
    {
        error = string.Empty;
        if (string.IsNullOrWhiteSpace(raw))
        {
            foreach (var item in items) item.CatalogDescription = null;
            return true;
        }
        foreach (var item in items) item.CatalogDescription = raw;
        return true;
    }

    private static bool ApplyBulkRowguid(IReadOnlyList<ProductModel> items, string? raw, out string error)
    {
        error = string.Empty;
    ...
    private static bool ApplyBulkModifiedDate(IReadOnlyList<ProductModel> items, string? raw, out string error)
    {
        error = string.Empty;
    ...
    private static IQueryable<ProductModel> ApplySearch(IQueryable<ProductModel> query, string? search)
    {
        if (string.IsNullOrWhiteSpace(search)) return query;
        search = search.Trim();
        return query.Where(x => (x.Name != null && x.Name.Contains(search)) || (x.CatalogDescription != null && x.CatalogDescription.Contains(search)));
    }

    private static IQueryable<ProductModel> ApplyFilter(IQueryable<ProductModel> query, string? filterField, string? filterValue)
    {
        ...
        return filterField.ToLowerInvariant() switch
        {
            ...
            "catalogdescription" => query.Where(x => x.CatalogDescription != null && x.CatalogDescription.Contains(filterValue)),
    ...
    private static IQueryable<ProductModel> ApplySort(IQueryable<ProductModel> query, string? sortBy, string? sortDirection)
    {
        ...
        return field.ToLowerInvariant() switch
        {
            ...
            "catalogdescription" => descending ? query.OrderByDescending(x => x.CatalogDescription) : query.OrderBy(x => x.CatalogDescription),
    ...
    private static ProductModelDto ToDto(ProductModel item) => new(
        item.ProductModelID,
        item.Name,
